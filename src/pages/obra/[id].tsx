...
export default function ObraPage() {
  ...
  if (isError) {
    return (
      <div className={home.app}>
        <main className={home.container} style={{ padding: '3rem 1rem' }}>
          <p style={{ color: 'var(--text-danger)' }}>Obra não encontrada.</p>
  ...
  if (isLoading || !obra) {
    return (
      <div className={home.app}>
        <main className={home.container} style={{ padding: '3rem 1rem', textAlign: 'center', color: 'var(--text-muted)' }}>
  ...
  return (
    <>
      <Head>
        <title>{obra.nome} — ObrasBH</title>
        <meta name="description" content={`Contrato, renovações, medições e situação da obra ${obra.nome} em Belo Horizonte`} />
      </Head>

      <div className={home.app}>
        <header className={styles.headerCompacto}>
          <div className={home.container}>
            <Link href="/" className={styles.logo}>ObrasBH</Link>
          </div>
        </header>
        ...
        <footer className={home.footer}>
          <p><strong>ObrasBH</strong> — Transparência para uma cidade melhor</p>
          <p>Dados sincronizados do painel Transparência Obras Públicas da SMOBI/PBH.</p>
        </footer>
      </div>
    </>
  )
